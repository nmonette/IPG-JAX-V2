            ...
            avg_train_states = all_train_states.replace(
                team_train_state = all_train_states.team_train_state.replace(
                    params = jax.tree_util.tree_map(
                        lambda prev, x: jax.vmap(jnp.divide)(prev + x.cumsum(axis=0), jnp.ones(x.shape[0]).cumsum() + t), sum_train_states.team_train_state.params, all_train_states.team_train_state.params
                    )
                ),
                adv_train_state = all_train_states.adv_train_state.replace(
                    params = jax.tree_util.tree_map(
                        lambda prev, x: jax.vmap(jnp.divide)(prev + x.cumsum(axis=0), jnp.ones(x.shape[0]).cumsum() + t),  sum_train_states.adv_train_state.params, all_train_states.adv_train_state.params
                    )
                )
            )
